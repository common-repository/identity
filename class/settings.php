...
class MCI_Identity_Settings {
	 ...
	 * @var bool
	 */
	const C_ARR_SLUGS = "identity_plugin_settings_container";
	 ...
	 * @var array
	 */
	private $a_arr_Container = array("identity_plugin_settings_container");
	...
	private $a_arr_Default = array(
        "identity_plugin_settings_container" => array(
			self::C_ARR_SLUGS => array()
		)
	);
    ...
    public function getAllTags() {
        ...
        foreach($this->a_arr_Settings as $l_str_Key => $l_str_Value) {
            // get only base unique ID (hidden input field) and ignore all other
            if (substr($l_str_Key, -1) === "-") {
                $l_arr_Tags[$l_str_Key] = $l_str_Key;
            }
        }
        return $l_arr_Tags;
    }
