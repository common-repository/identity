 ...
 * @since  1.5.0
 */
abstract class MCI_Identity_LayoutEngine {
	...
	protected function addSection($p_str_ID, $p_str_Title, $p_int_SettingsContainerIndex, $p_bool_hasSubmitButton = true) {
		return array("id" => MCI_Identity_Config::C_STR_PLUGIN_NAME . "-" . $p_str_ID, "title" => $p_str_Title, "submit" => $p_bool_hasSubmitButton, "container" => $p_int_SettingsContainerIndex);
	}
	...
	protected function addMetaBox($p_str_SectionID, $p_str_ID, $p_str_Title, $p_str_CallbackFunctionName) {
		return array("parent" => MCI_Identity_Config::C_STR_PLUGIN_NAME . "-" . $p_str_SectionID, "id" => $p_str_ID, "title" => $p_str_Title, "callback" => $p_str_CallbackFunctionName);
	}
	...
	public function registerSubPage() {
        ...
        if (array_key_exists(plugin_basename(MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG), $submenu)) {
            // iterate through all sub menu entries of the ManFisher main menu
            foreach($submenu[plugin_basename(MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG)] as $l_arr_SubMenu) {
                if ($l_arr_SubMenu[2] == plugin_basename(MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG . $this->getSubPageSlug())) {
                    // remove that sub menu and add it again to move it to the bottom
                    remove_submenu_page(MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG, MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG .$this->getSubPageSlug());
                }
            }
        }

        $this->a_str_SubPageHook = add_submenu_page(
            MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG, // parent slug
            $this->getSubPageTitle(), // page title
            $this->getSubPageTitle(), // menu title
            'manage_options', // capability
            MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG . $this->getSubPageSlug(), // menu slug
	...
	private function appendScripts() {
		...
		// register stylesheet
		wp_register_style('mci-identity-admin-styles', plugins_url('../../css/settings.css', __FILE__));
		// add stylesheet to the output
		wp_enqueue_style('mci-identity-admin-styles');
	}
	...
	public function displayContent() {
		...
		foreach ($this->a_arr_Sections as $l_str_ID => $l_arr_Description) {
			echo sprintf(
				...
				MCI_Identity_Layout_Init::C_STR_MAIN_MENU_SLUG . $this->getSubPageSlug(), $l_str_ID, $l_arr_Description["title"]
			);
		}
		echo '</h2><br/>';

		if ($l_bool_SettingsUpdated) {
			echo sprintf('<div id="message" class="updated">%s</div>', __("Settings saved", MCI_Identity_Config::C_STR_PLUGIN_NAME));
		}
	...
	private function saveSettings() {
		...
		$l_arr_ActiveSection = $this->a_arr_Sections[$l_str_ActiveSectionID];
		// update settings
		return MCI_Identity_Settings::instance()->saveOptions($l_arr_ActiveSection["container"], $_POST);
	}
	...
	protected function LoadSetting($p_str_SettingKeyName) {
		...
		$p_arr_Return["value"] = esc_attr(MCI_Identity_Settings::instance()->get($p_str_SettingKeyName));
		return $p_arr_Return;
	}
	 ...
	 * @param string $p_str_ClassName Optional class name.
	 ...
	 * @return string
	 */
	protected function addTextBox($p_str_SettingName, $p_str_ClassName = "", $p_bool_Readonly = false, $p_bool_Hidden = false) {
		$l_str_Style = "";
		...
		return sprintf('<input type="text" name="%s" id="%s" class="%s" style="%s" value="%s" %s/>',
					   $l_arr_Data["name"], $l_arr_Data["id"], $p_str_ClassName,
	...
	protected function addCheckbox($p_str_SettingName) {
		...
		return sprintf('<input type="checkbox" name="%s" id="%s" %s/>',
					   ...
					   MCI_Identity_Convert::toBool($l_arr_Data["value"]) ? 'checked="checked"' : '');
	}
