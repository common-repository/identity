 ...
 * @since  1.5.0
 */
class MCI_Identity_Layout_Init {
	...
	public function __construct() {
		...
		foreach(get_declared_classes() as $l_str_ClassName) {
			...
			if(is_subclass_of($l_str_ClassName, 'MCI_Identity_LayoutEngine')) {
				/** @var MCI_Identity_LayoutEngine $l_obj_Class */
		...
		add_action("wp_ajax_nopriv_identity_getPluginInfo", array($this, "getPluginMetaInformation"));
		add_action("wp_ajax_identity_getPluginInfo", array($this, "getPluginMetaInformation"));
	}
	...
	public function initializeSettings() {
		MCI_Identity_Settings::instance()->RegisterSettings();
		// iterate though each sub class of the layout engine and register their sections
		/** @var MCI_Identity_LayoutEngine $l_obj_LayoutEngineSubClass */
	...
	public function registerMainMenu() {
		...
		add_menu_page(
			...
			plugins_url('identity/img/main-menu.png'), // icon url
			null // position
		);
		$this->registerSubPages();
	}
	...
	public function displayOtherPlugins() {
		printf("<br/><br/>");
		// load template file
		$l_obj_Template = new MCI_Identity_Template(MCI_Identity_Template::C_STR_DASHBOARD, "manfisher");
		...
		printf("<br/><br/>");
		printf("<h3>%s</h3>", __('Take a look on other Plugins we have developed.', MCI_Identity_Config::C_STR_PLUGIN_NAME));
		...
		if (is_wp_error($l_arr_Response)) {
			printf(__("Error loading other WordPress Plugins from Manfisher. Sorry!", MCI_Identity_Config::C_STR_PLUGIN_NAME));
			return;
		}
		...
		// load template file
		$l_obj_Template = new MCI_Identity_Template(MCI_Identity_Template::C_STR_DASHBOARD, "other-plugins");

		printf('<div id="the-list">');
		// iterate through each Plugin
		foreach($l_arr_Plugins as $l_arr_PluginInfo) {
            $l_str_InstallButton = '<a class="install-now button" href="'.$l_str_Server.'/wp-admin/update.php?action=install-plugin&plugin='.$l_arr_PluginInfo["name"].'&_wpnonce=e7a5c90faf" aria-label="Install '.$l_arr_PluginInfo["title"].' now">'.__("Install now", MCI_Identity_Config::C_STR_PLUGIN_NAME).'</a>';
            $l_str_AlreadyInstalled = '<span class="button button-disabled" title="'.__("This Plugin is already installed and up to date.", MCI_Identity_Config::C_STR_PLUGIN_NAME).'">'.__("Installed", MCI_Identity_Config::C_STR_PLUGIN_NAME).'</span>';
			...
			$l_obj_Template->replace(
				array(
                    ...
                    "more-details-label" => __("More Details", MCI_Identity_Config::C_STR_PLUGIN_NAME),
                    "last-updated-label" => __("Last Updated", MCI_Identity_Config::C_STR_PLUGIN_NAME)
				)
			);
			// display Plugin
			echo $l_obj_Template->getContent();
	...
	public function getPluginMetaInformation() {
		...
		echo json_encode(
			array(
				...
				"PluginRatingText" => $l_int_Stars . " " . __("rating based on", MCI_Identity_Config::C_STR_PLUGIN_NAME) . " " . $l_int_NumRatings . " " . __("ratings", MCI_Identity_Config::C_STR_PLUGIN_NAME),
