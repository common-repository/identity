 ...
 * @since 1.5.0
 */
class MCI_Identity_Layout_Diagnostics extends MCI_Identity_LayoutEngine {
	...
	protected function getSubPageTitle() {
		return __("Diagnostics", MCI_Identity_Config::C_STR_PLUGIN_NAME);
	}
	...
	protected function getSections() {
		return array(
			$this->addSection("diagnostics", __("Diagnostics", MCI_Identity_Config::C_STR_PLUGIN_NAME), null, false)
		);
	}
	...
	protected function getMetaBoxes() {
		return array(
			$this->addMetaBox("diagnostics", "diagnostics", __("Displays information about the web server, PHP and WordPress", MCI_Identity_Config::C_STR_PLUGIN_NAME), "Diagnostics")
		);
	}
	...
	public function Diagnostics() {
		...
		// load template file
		$l_obj_Template = new MCI_Identity_Template(MCI_Identity_Template::C_STR_DASHBOARD, "diagnostics");
		// replace all placeholders
		$l_obj_Template->replace(
			array(
				"label-server" => __("Server name", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"server" => $_SERVER["SERVER_NAME"],

				"label-php" => __("PHP version", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"php" => phpversion(),

                "label-user-agent" => __("User agent", MCI_Identity_Config::C_STR_PLUGIN_NAME),
                "user-agent" => $_SERVER["HTTP_USER_AGENT"],

				"label-max-execution-time" => __("Max execution time", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"max-execution-time" => ini_get('max_execution_time') . ' ' . __('seconds', MCI_Identity_Config::C_STR_PLUGIN_NAME),

				"label-memory-limit" => __("Memory limit", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"memory-limit" => ini_get('memory_limit'),

				"label-php-extensions" => __("PHP extensions", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"php-extensions" => $l_str_PhpExtensions,

				"label-wordpress" => __("WordPress version", MCI_Identity_Config::C_STR_PLUGIN_NAME),
				"wordpress" => $wp_version,

				"label-theme" => __("Active Theme", MCI_Identity_Config::C_STR_PLUGIN_NAME),
